"""Evaluate 3-segment (SEQ3) within-chunk ordering. Extends eval_seq_temporal to
thirds of the 18-action chunk. For a plan "go a, then b, then c", each third should
steer toward a/b/c on the relevant stick axis. Tests whether the order-aware
contrastive fix (EXP-016) scales beyond 2 sub-segments.
"""
import os, sys, glob
import numpy as np
import torch
...
# SEQ3 plans match plans.py: (a, b, c)
SEQ3 = {
    "left,up,right": ("go left, then up, then right", ["left", "up", "right"]),
    "up,right,down": ("go up, then right, then down", ["up", "right", "down"]),
    "down,left,up":  ("go down, then left, then up", ["down", "left", "up"]),
}
...
def evaluate(tag, path, which, null_mode="masked"):
    ...
    print(f"\n=== {tag} ===")
    thirds = [(0, 6), (6, 12), (12, 18)]
    for name, (text, dirs) in SEQ3.items():
        seg = np.zeros(3)
        for png in pngs:
            for t in TS:
        ...
        seg /= (len(pngs) * len(TS))
        oks = [int(np.sign(seg[i]) == AX[dirs[i]][1]) for i in range(3)]
        msg = "  ".join(f"{dirs[i]}:{seg[i]:+.3f}({'+' if AX[dirs[i]][1]>0 else '-'}){'OK' if oks[i] else '✗'}" for i in range(3))
        print(f"  {name:16s} {msg}   [{sum(oks)}/3]")
